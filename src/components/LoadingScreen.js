...
const LoadingScreen = () => {
  ...
  useEffect(() => {
    ...
    const animateText = () => {
      ...
      // Append "Linus Elvius> " without animation
      const prefixSpan = document.createElement('span');
      prefixSpan.textContent = "linuselvius> ";
    ...
    const animateNextText = (index) => {
      if (index < text.length) {
        ...
        gsap.to(cursorSpan, { opacity: 0, repeat: -1, yoyo: true, duration: 0.5 });
        // Fade in the button
  ...
  return loadingScreenVisible ? (
    <div className="loading-screen" ref={terminalRef}>
      <button ref={buttonRef} style={{ opacity: 0 }} id='access-btn' onClick={handleButtonClick}>Continue</button>
    </div>
  ) : <App />;
};
